...
/**
 * Extracts blob references from a block
 */
...
export async function cacheLeafletImages(
  handle: string,
  records: Array<{ uri: string; value: { content: { pages: Array<{ blocks: BlockWrapper[] }> } } }>,
  ...
  try {
    ...
    for (const record of records) {
      const blockWrappers = record.value.content?.pages[0]?.blocks || [];

      for (const wrapper of blockWrappers) {
        const block = wrapper.block;
        if (block) {
          allBlobs.push(...extractBlobsFromBlock(block as unknown as Record<string, unknown>));
        }
      }
    }
